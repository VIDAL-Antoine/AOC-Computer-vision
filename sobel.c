...
void sobel_v2(u8 *cframe, u8 *oframe, i32 threshold)
{
  ...
  for (u64 i = 0; i < (H - 3); i++)
  {
    for (u64 j = 0; j < ((W * 3) - 3); j++)
    {
      ...
      cframe_tmp = cframe[i_W3_j + 2];
      gx += cframe_tmp;
      gy -= cframe_tmp;
      
      cframe_tmp = cframe[i_W3_j + 6];
      gx -= cframe_tmp;
      gy += cframe_tmp;
      
      cframe_tmp = cframe[i_W3_j + 8];
      gx += cframe_tmp;
      gy += cframe_tmp;
      
      cframe_tmp = (cframe[i_W3_j + 5] - cframe[i_W3_j + 3]);
      cframe_tmp *= 2;
      gx += cframe_tmp;
      
      cframe_tmp = (cframe[i_W3_j + 7] - cframe[i_W3_j + 1]);
      cframe_tmp *= 2;
      gy += cframe_tmp;
